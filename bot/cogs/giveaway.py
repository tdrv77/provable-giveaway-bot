    ...
    async def _interactive_setup(self, context):
        ...
        while True:
            ...
            if perms.send_messages is False:
                await context.say_as_embed(
                    f'I can\'t send messages in {channel.mention}! Please try again.',
                    delete_after=5, color='error'
                )
                continue
        ...
        if resp in ['n', 'no']:
            await context.say_as_embed(title='Giveaway Creation Ended.')
            return

        ga_obj = Giveaway.objects.create(**ga_data)

        try:
            ga_message = await ga_channel.send(embed=ga_obj.ga_embed)
